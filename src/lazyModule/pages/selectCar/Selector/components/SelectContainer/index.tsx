...
const SelectorContainer = (
  props: SelectorContainerPropsType
): React.ReactElement => {
  ...
  useReady(() => {
    setTimeout(() => {
      selectorQueryClientRect("." + style.main).then(res => {
  ...
  const handleClickDropdownWrapper = () =>
    activeId !== null && setActiveId(null);
    ...
    <>
      <View className={style.main} ref={mainRef}>
        {items.map(item => (
          <View
            ...
            className={[
              style.selectorItem,
              activeId === item.id ? style.activeItem : ""
            ].join(" ")}
            key={item.id}
          >
            {item.name}
      ...
      <View
        className={[
          ...
          activeId === null ? style.hidden : style.show
        ].join(" ")}
        ref={dropdownRef}
