  ...
  const fetchOrderData = () => {
    ...
    getOrderById(parseInt(params.id))
      .then(res => {
        setOrder(res);
        taro
          .setNavigationBarTitle({ title: `订单号 ${res.alipayTradeNo}` })
          .then(() => console.log("set order detail title"));
      })
      ...
      {!!order && (
        <View className={style.main}>
          ...
          <View className={style.bottomWrapper}>
            <BottomMenuBarRender order={order} />
          </View>
        </View>
      )}
      {reletPanel && (
        <SpinContainer>
          <PanelRenderer
            order={order}
            onCancel={() => setReletPanel(false)}
            onSubmit={handleReletSubmit}
          />
        </SpinContainer>
      )}
    </>
