...
const PromotionPage: React.FC = () => {
  const { promotionLevel2, promotionLevel1 } = useAppSelector(
  ...
  useEffect(() => {
    getPromotionInfo().then(res => setPromotionInfo(res));
  }, []);
  const [loading, setLoading] = useState<boolean>(false);
  const me = useAppSelector(state => state.me);
  const [isWithDraw, setIsWithDraw] = useState<boolean>(false);
  const dispatch = useAppDispatch();
  const handleWithdraw = async (v: TopUpValueType) => {
    if (v.amount < withDrawLimit) {
      await taro.showToast({ title: `提现金额不能小于${withDrawLimit}` });
      return;
    }
    setLoading(true);
    try {
      await createWithdraw({ ...v, isCommission: true });
  ...
  const handleShowWithdrawPanel = async () => {
    if (me?.data?.commission < withDrawLimit) {
      await taro.showToast({ title: `佣金不足${withDrawLimit},无法提现` });
    } else {
      setIsWithDraw(true);
    }
  };
      ...
      <View className={style.main}>
        <View className={style.commissionWrapper}>
          <View className={style.balanceWrapper}>
            <View className={style.balance}>
              {me.data?.commission || "0.00"}
            </View>
            <View className={style.withDraw} onClick={handleShowWithdrawPanel}>
              提现
            </View>
          </View>
          <View>佣金(元)</View>
        ...
        <View className={style.menuBarWrapper}>
          <View className={style.itemWrapper}>
            <View>
              已提现佣金:{" "}
              <Text className={style.balance}>
                {me.data?.withdrawnCommission || "0.00"}
              </Text>
            </View>
            <View>
              提现中佣金:{" "}
              <Text className={style.balance}>
                {me.data?.withdrawalInProgressCommission || "0.00"}
              </Text>
            </View>
          </View>
          <View className={style.itemWrapper}>
            <View>
              一级返点:{" "}
              <Text className={style.rebatePoints}>{promotionLevel1}%</Text>
            </View>
            <View>
              二级返点:{" "}
              <Text className={style.rebatePoints}>{promotionLevel2}%</Text>
            </View>
          </View>
          <View
        ...
        <View className={style.ordersWrapper}>
          ...
          {promotionInfo.orders.map(order => (
        ...
        <View className={style.promotionWrapper}>
          ...
          >
            <Icon value="promotion" className={style.icon} />
            <View>去推广</View>
          </Button>
        </View>
      </View>
