...
const PromotionUsersPage: React.FC = () => {
  ...
  const [active, setActive] = useState<ActiveType>(ActiveType.PROMOTION_LEVEL_1_USERS)
  useEffect(() => {
    setLoading(true)
    getPromotionInfo()
  ...
  if (active === ActiveType.PROMOTION_LEVEL_1_USERS) {
    userItemsDom = promotionInfo.promotionLevel1Users.map(user => (<UserItemRender data={user} key={user.id} />))
  } else {
    userItemsDom = promotionInfo.promotionLevel2Users.map(user => (<UserItemRender data={user} key={user.id} />))
  }
  ...
  return (<>
    ...
    <View className={style.main}>
      <View className={style.tabWrapper}>
        <Button
          {...( active === ActiveType.PROMOTION_LEVEL_1_USERS ?  {type: 'primary' } : {})}
          onClick={() => setActive(ActiveType.PROMOTION_LEVEL_1_USERS)}
        >一级下线</Button>
        <Button
          {...( active === ActiveType.PROMOTION_LEVEL_2_USERS ?  {type: 'primary' } : {})}
