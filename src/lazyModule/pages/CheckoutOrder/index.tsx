...
const Order: React.FC = () => {
  ...
  const handleInitAmountList = () => {
    ...
    const insuranceFee: number = isInsuranceFee
      ? car.insuranceFee * insuranceFeeDays
      : 0;
    ...
    if (
      userCoupon &&
      userCoupon.isValid &&
      rent + handlingFee + insuranceFee >= userCoupon.meetAmount &&
      userCoupon.amount > 0
    ) {
  ...
  const { idCarBack, idCarFrontal, driverLicense } =
    useAppSelector(state => state.me?.data) || {};
  ...
  return (
    <>
        ...
        <SpinContainer className={style.spin}>
          <View className={style.listenWrapper}>
            ...
            <View className={style.button}>
              <Button type="primary" onClick={handleCloseSpin}>
                确认
              </Button>
            </View>
          </View>
      ...
      <View className={style.main}>
        <MenuContainer ref={mainRef} menuBar={menuRender} bodyHeight={88}>
          ...
          <View className={style.container}>
            ...
            <OrderAgreement />
            <CheckRender checked={allowed} onChange={setAllowed} />
          </View>
        </MenuContainer>
      </View>
    </>
