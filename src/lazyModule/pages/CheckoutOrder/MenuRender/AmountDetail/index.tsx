...
const AmountDetail: React.FC<AmountDetailPropsType> = ({
  amount,
  onClose,
  depositItems,
  payNowItems
  ...
  return (
    <>
      <SpinContainer className={style.spinWrapper} onClick={onClose}>
        <View className={style.spinContainer}>
          <View className={style.header}>
            <Icon value="close" className={style.icon} onClick={onClose} />
            <View>费用明细</View>
            <View className={style.rule} onClick={handleShowAgreement}>
              费用规则
            </View>
          </View>
          <View className={style.itemsWrapper}>
            {payNowItems.map(item => (
              <View className={style.itemWrapper} key={item.title}>
                <View className={style.title}>{item.title}</View>
                <View className={style.amount}>
                  {!!item.waiverFee && (
                    <Text className={[style.wrapperAmount].join(" ")}>
                      ￥{Math.round(item.waiverFee * 100) / 100}
                    </Text>
                  )}
                  <Text className={item.activeColor ? style.activeAmount : ""}>
          ...
          <View className={style.itemsWrapper}>
            ...
            <View
              className={[style.itemWrapper, style.amountItemWrapper].join(" ")}
            >
              <View className={style.title}>订单总额</View>
              <View className={style.amount}>
                ￥
                <Text className={style.amountNumber}>{amount.toFixed(2)}</Text>
              </View>
            </View>
          </View>
        </View>
