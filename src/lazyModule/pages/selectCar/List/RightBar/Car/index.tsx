    ...
    <View className={style.main} onClick={handleClick}>
      <View className={style.firstRowWrapper}>
        ...
        <View className={style.leftInfoWrapper}>
          <View className={style.nameWrapper}>
            ...
            {props.data.number && (
              <View className={style.license}>
                {props.data.number.substring(0, 2)}
              </View>
            )}
          </View>
          <View className={style.configurationWrapper}>
            {props.data.shift === "MANUAL" ? "手动" : "自动"}
            {props.data.gasVolume}L/
            {props.data.seats}座/
          ...
          <View className={style.tagWrapper}>
            {props.data.tags.map((tag, key) => (
              <View className={style.tag} key={key}>
                {tag}
              </View>
            ))}
          </View>
      ...
      <View className={style.secondRowWrapper}>
        ...
        >
          <View>
            {" "}
            <Text className={style.satisfaction}>100%</Text>满意{" "}
          </View>
