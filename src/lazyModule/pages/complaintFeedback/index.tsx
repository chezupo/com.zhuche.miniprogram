...
const ComplaintFeedback = (): React.ReactElement => {
  ...
  const handleChangePhone = e => setData(() => ({...data, phone: e.currentTarget.value}))
  const handleChangeEmail = e => setData(() => ({...data, email: e.currentTarget.value}))
  const handleChangeContent = (e) => setData(() => ({...data, content: e.currentTarget.value}))
      ...
      <View className={style.main}>
        ...
        >
          <>
            {tags.map((tag, key) => (
              <View
                className={[style.button, tag.name ===  data.flag ? style.activeTag : ''].join(' ')}
        ...
        <View className={style.buttonWrapper}>
          <View className={style.submitButton}
