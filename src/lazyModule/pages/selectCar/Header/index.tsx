...
const Header = (): React.ReactElement => {
  const {startStore, starTime: startTimestamp, endTime: endTimeStamp} = useAppSelector(state => state.order.createOrder)
  ...
  return (
    <View className={style.headerWrapper}>
      <View>{startStore.name}
        {startStore.mark.length > 0 ? <>{`(${startStore.mark})`}</> : <></>}
      </View>
      <View className={style.dateRange}>
        <View className={style.date}>{format(month)}-{format(date)} {format(hours)}:{format(minutes)}</View>
        <View className={style.dateRangeBetweenBorder} />
        <View className={[style.date, style.endDate].join(' ')}>
          <View> {format(endTime.month)}-{format(endTime.date)} {format(endTime.hours)}:{format(endTime.minutes)} </View>
