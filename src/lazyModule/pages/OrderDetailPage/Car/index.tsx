...
import {navigateStoreDetail, navigateToCarDetailPage} from "../../../../store/module/router";

const Car: React.FC<OrderPropsType> = ({order}) => {
  const handleDetail = () => {
    navigateToCarDetailPage(order.car.id)
  }
  const handleClickLocation = (latitude: number, longitude: number, address: string, name: string) => {
    taro.openLocation({
      longitude,
      latitude,
      name,
    ...
    <Card>
      <View className={style.carInfoWrapper}>
        ...
        <View className={style.infoWrapper}>
          ...
          <View>{order.car.type} / {order.car.seats}座 / {order.car.gasVolume}L</View>
        </View>
      </View>
      <View className={style.addressItemWrapper}>
        <View className={style.itemWrapper}>
          <View className={style.address}>
            ...
            <View onClick={() => navigateStoreDetail(order.startStore.id, true)} >
              <View>店员免费送车上门</View>
              <View>{order.startStore.address}</View>
            </View>
          </View>
          <Icon value='location' className={style.location}
            onClick={
            () => handleClickLocation(
        ...
        <View className={style.itemWrapper}>
          <View className={style.address}>
            ...
            <View onClick={() => navigateStoreDetail(order.endStore.id, true)} >
              <View>店员免费送车上门</View>
