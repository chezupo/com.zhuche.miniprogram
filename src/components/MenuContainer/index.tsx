...
const MenuContainer: React.FC<MenuContainerPropsType> = props => {
  ...
  const [bodyStyle, bottomStyle]: [CSSProperties, CSSProperties] = bodyHeight
    ? [{ height: `${bodyHeight}vh` }, { height: `${100 - bodyHeight}vh` }]
    : [{}, {}];
  return (
    <>
      <View
        ref={props.ref}
        style={bodyStyle}
        className={[style.bodyWrapper, props.bodyClassName || ""].join(" ")}
      >
        {props.children}
      </View>
      <View
