...
const PanelRenderer: React.FC<PanelRendererPropsType> = ({order, ...props}) => {
  ...
  const setRelateData = () => {
    if (getDay()) {
      setAmount( Math.round(order.car.rent * getDay() * 100)  / 100.0 )
      setExpired( order.endTimeStamp + getDay() * 60 * 60 * 24 * 1000 )
    }
  }
  useEffect(() => setRelateData() , [value, days])
  const timestampCoverToDateStr = (): string => convertDateToStr( new Date(expired) )
