...
export type AmountListType = Pick<
  CarItemType,
  "insuranceFee" | "handlingFee" | "deposit" | "rent"
> & {
...
const MenuRender: React.FC<MenuRenderPropsType> = ({
  amountList,
  ...props
}) => {
  ...
  const handleInit = () => {
    setPayNowItems([
      {
        ...
        waiverFee: amountList.waiverRent
          ? amountList.waiverRent + amountList.rent
          : 0
      },
      {
        title: "手续费",
        amount: amountList.handlingFee,
        waiverFee: amountList.waiverHandlingFee
          ? amountList.handlingFee + amountList.waiverHandlingFee
          : 0
      },
      ...
      ...(amountList.waiverHandlingFee + amountList.waiverRent > 0
        ? [
            {
              title: "优惠卷减免",
              amount: -(amountList.waiverHandlingFee + amountList.waiverRent),
              activeColor: true
            }
          ]
        : [])
    ...
    setAmount(
      amountList.rent + amountList.handlingFee + amountList.insuranceFee
    );
  };
      ...
      case AllPlatformType.WECHAT:
        return (
          <Button
            ...
            onGetPhoneNumber={e =>
              handleGetWechatPhone(e.detail.iv, e.detail.encryptedData)
            }
            onError={e => {
              setPopErrorVisible(true);
            }}
      ...
      <View className={style.main}>
        ...
        <View className={style.container}>
          <View>
            ...
            <Text className={style.themeColor}>
              ￥<Text className={style.amount}>{amount.toFixed(2)}</Text>
            </Text>
          </View>
