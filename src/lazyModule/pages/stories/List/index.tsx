...
const List = (): React.ReactElement => {
  ...
  const [popularAttractions] = useObserve(
    useAppStoreSelector().popularAttractions
  );
  const { createOrder } = useAppSelector(state => state.order);
  let cityStores: AreaStoreType[] =
    createOrder.startStoreOrEndStore === StartStoreOrEndStoreType.START
      ? createOrder.starCityStores
      : createOrder.endCityStores;
  const items: AreaType[] = [
    defaultActiveItem,
    ...cityStores.map(({ name, code }) => ({ name, code }))
  ];
  const codeMapAreaStore: Map<string, AreaStoreType> = new Map<
    string,
    AreaStoreType
  >();
      ...
      <View className={style.leftBar}>
        {items.map((item, key) => (
          <view
            ...
            className={[
              style.itemWrapper,
              activeArea.code === item.code ? style.activeItem : ""
            ].join(" ")}
            key={item.code}
