...
import { navigateToOrderDetailPage } from "../../../store/module/router";
import { getOrderById, tradeOverTimeOrderById } from "../../../api/order";
  ...
  const handleReturnCar = async () => {
    ...
    if (order.expiredFee > 0) {
      const res = await taro.showModal({
        title: `您已超时用车${props.data.expiredDays.toFixed(
          1
        )}天, 需补交${props.data.expiredFee.toFixed(2)}元, 是否补交并还车?`
      });
    ...
    <>
      <View className={style.main}>
        <View className={style.headerWrapper}>
          <View className={style.status}>{getStatusMapInfo(data)}</View>
          <View>￥{props.data.amount}</View>
        </View>
        <View className={style.orderNo} onClick={handleCopy}>
          <View>订单号: {props.data.outTradeNo}</View>
          <Icon className={style.copyIcon} value="copy" />
        </View>
        ...
        <View className={style.storeWrapper}>
          <View className={style.storeAddressWrapper}>
            ...
            <View className={style.storeAddress}>
              <View>
                {data.startStore.city.name} {data.startStore.name}
              </View>
              <View>
                {data.endStore.city.name} {data.endStore.name}
              </View>
            </View>
          </View>
        ...
        <View className={style.buttonWrapper}>
          {["USING", "OVERTIME"].includes(props.data.status) && (
            <View className={style.button} onClick={handleReturnCar}>
              还车
            </View>
          )}
          {["FINISHED", "CANCELED"].includes(props.data.status) && (
            <View className={style.button} onClick={onDeleteOrder}>
              删除订单
            </View>
          )}
          {payStatus.includes(props.data.status) && (
            <View className={style.button} onClick={handlePay}>
              支 付
            </View>
          )}
          {/*{*/}
          ...
          {[
            ...
            <View
              ...
              onClick={() => handleRebook(props.data)}
            >
              再次预订
            </View>
          )}
          {["FINISHED"].includes(props.data.status) && !props.data.comment && (
            <View
              className={style.button}
              onClick={() => props.onComment(props.data)}
            >
              评价
            </View>
          )}
