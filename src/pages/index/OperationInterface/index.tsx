...
const OperationInterface = (
  props: OperationInterfacePropsType
): React.ReactElement => {
  ...
  const handleRedirect = (
    ...
    setStartStoreOrStoreCity: StartStoreOrEndStoreType
  ) => {
    if (cityType === null) {
  ...
  return (
    <View
      className={[style.main, props.className ? props.className : ""].join(" ")}
    >
      <View className={style.container}>
        <SelectCityOrAttraction
          ...
          storeName={
            createOrder.startStore?.name
              ? `${createOrder.startStore?.name}${
                  createOrder.startStore.mark
                    ? `(${createOrder.startStore.mark})`
                    : ""
                }`
              : ""
        ...
        {createOrder.isForeignCity && (
          <SelectCityOrAttraction
            ...
            storeName={
              createOrder.endStore?.name
                ? `${createOrder.endStore?.name}${
                    createOrder.endStore.mark
                      ? `(${createOrder.endStore.mark})`
                      : ""
                  }`
                : ""
