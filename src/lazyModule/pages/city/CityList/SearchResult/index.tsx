...
import {
  CategoryMapCitiesType,
  CityCategory,
  pickCity
} from "../../../../../store/module/cities";
...
const ResultRender = (): React.ReactElement => {
  ...
  const [categoryMapCities] = useObserve<CategoryMapCitiesType>(
    useAppStoreSelector().city.cityCategoriesObserve
  );
  ...
  categoryMapCities.forEach((list, cityCategory) => {
    ...
    list.forEach(cityInfo => {
      ...
      const isMatch =
        !!cityInfo.name.match(regName) || !!cityInfo.pinyin.match(regName);
