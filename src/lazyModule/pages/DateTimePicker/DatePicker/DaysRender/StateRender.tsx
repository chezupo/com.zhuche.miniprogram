...
const StateRender: React.FC<StateRenderPropsType> = props => {
  ...
  useEffect(() => {
    ...
    if (
      props.date &&
      timeToDateTime(new Date()).getTime() ===
        timeToDateTime(props.date).getTime()
    ) {
      newDate = "今";
      setisToday(true);
      setNumber(newDate);
  ...
  const handleInitLastDate = () => {
    ...
    const lastDate = new Date(
      timeToMonthTime(nextMonth).getTime() - 1
    ).getDate();
  ...
  return (
    <>
      ...
      />
      <View className={[style.radiusNumberWrapper].join(" ")}>
        <View
          className={[
            ...
            isToday ? style.today : "",
            (isSunday || isSaturday || isStartMonthDate || isLastMonthDate) &&
            props.isRange
              ? style.isSunday
              : ""
          ].join(" ")}
