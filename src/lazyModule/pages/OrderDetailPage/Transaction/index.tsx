...
export const getDays = (v: OrderItemType) =>
  ((v.endTimeStamp - v.startTimeStamp) / (60 * 60 * 24 * 1000)).toFixed(1);
...
const Transaction: React.FC<OrderPropsType &
  OrderCancelPropsType & {
  ...
  useEffect(() => {
    setPayNowItems([
      ...
      ...(order.insuranceFee
        ? [{ title: "驾无忧", amount: order.insuranceFee }]
        : []),
      ...(order.waiverHandlingFee + order.waiverRent > 0
        ? [
            {
              title: "优惠卷减免",
              amount: -(order.waiverHandlingFee + order.waiverRent),
              activeColor: true
            }
          ]
        : [])
  ...
  return (
    <>
      {!!contract && (
        <SpinContainer onClick={() => setContract(undefined)}>
          <Image src={contract} />
        </SpinContainer>
      )}
      <Card>
        <View className={style.main}>
          ...
          <View className={style.itemWrapper}>
            <View>
              <View>
                押金: <Text className={style.flag}>免收</Text>
              </View>
            </View>
            <View className={style.freeze} onClick={handleShowAmountDetail}>
              免押明细
            </View>
          </View>
          <View className={style.itemWrapper}>
            ...
            {!!order?.contract && (
              <View className={style.freeze} onClick={handleShowContract}>
                取车合同、取车验车单
              </View>
            )}
          </View>
          ...
          <View className={style.actionWrapper}>
            ...
            {["CANCELED", "FINISHED"].findIndex(e => e == order.status) !==
              -1 && (
              <Button
                className={style.button}
                type="primary"
