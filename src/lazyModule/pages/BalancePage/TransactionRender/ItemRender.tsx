        ...
        <View className={style.rightWrapper}>
          <View className={style.amountWrapper}>
            ...
            {props.data.status === "PROCESSING" && (
              <View className={[style.notice, style.warningColor].join(" ")}>
                进行中
              </View>
            )}
            {props.data.status === "FAILED" && (
              <View className={[style.notice, style.failed].join(" ")}>
                失败
              </View>
            )}
          </View>
          <View className={style.subtitle}>
            余额 {(props.data.balance * 0.01).toFixed(2)}
          </View>
          {props.data.failedReason && (
            <View className={style.subtitle}>
              原因: {props.data.failedReason}
            </View>
          )}
        </View>
      </View>
